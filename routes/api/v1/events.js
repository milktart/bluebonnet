...
router.get('/:id', async (req, res) => {
  try {
    const { Event, Trip, TravelCompanion, ItemCompanion } = require('../../../models');
    ...
    const permissions = await getItemPermissions(event, userId);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await requireItemEditPermission(event, req.user.id, 'event');
