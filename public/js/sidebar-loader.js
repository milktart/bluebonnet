...
function initializeSidebarContent() {
  ...
  const deleteButtons = document.querySelectorAll(
    '#secondary-sidebar-content [data-action="delete"]'
  );
...
function goBackInSidebar() {
  if (sidebarHistoryIndex > 0) {
    ...
    eventBus.emit(EventTypes.SIDEBAR_HISTORY_CHANGED, {
      index: sidebarHistoryIndex,
    ...
    eventBus.emit(EventTypes.SIDEBAR_CLOSED, {});
  }
}
...
async function handleEventEditFormSubmit(e) {
  ...
  try {
    ...
    const eventIdMatch =
      document.getElementById('event-edit-form')?.getAttribute('data-event-id') ||
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.addEventListener('click', function (event) {
    ...
    const isOpenButton =
      event.target.closest('[onclick*="loadSidebarContent"]') ||
