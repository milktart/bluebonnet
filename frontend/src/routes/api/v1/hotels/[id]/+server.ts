...
export const GET: RequestHandler = async ({ params }) => proxyToBackend('GET', `/api/v1/hotels/${params.id}`);
export const PUT: RequestHandler = async ({ params, request }) => proxyToBackend('PUT', `/api/v1/hotels/${params.id}`, request);
export const DELETE: RequestHandler = async ({ params }) => proxyToBackend('DELETE', `/api/v1/hotels/${params.id}`);
