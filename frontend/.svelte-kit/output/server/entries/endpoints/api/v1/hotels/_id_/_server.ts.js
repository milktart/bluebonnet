...
const GET = async ({ params }) => proxyToBackend("GET", `/api/v1/hotels/${params.id}`);
const PUT = async ({ params, request }) => proxyToBackend("PUT", `/api/v1/hotels/${params.id}`, request);
const DELETE = async ({ params }) => proxyToBackend("DELETE", `/api/v1/hotels/${params.id}`);
export {
  DELETE,
  GET,
  PUT
